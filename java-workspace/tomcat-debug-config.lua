...
dap.configurations.java = {
  ...
  -- Remote Tomcat debugging (production/remote server)
  {
    ...
    type = "java",
    request = "attach",
    hostName = vim.fn.input("Remote host (localhost): ", "localhost"),
    port = vim.fn.input("Remote debug port (8000): ", "8000"),
    skipFiles = { "<rt jar>", "*/node_modules/**" },
    stopOnEntry = false,
    cwd = "${workspaceFolder}",
  },
  ...
  -- Maven Surefire test debugging
  {
    ...
    args = vim.fn.split(
      vim.fn.system("mvn help:describe -Dplugin=org.apache.maven.plugins:maven-surefire-plugin"),
      "\n"
    ),
    stopOnEntry = false,
    skipFiles = { "<rt jar>", "*/node_modules/**" },
  ...
  -- Application main class debugging
  {
    ...
    cwd = "${workspaceFolder}",
    console = "integratedTerminal",
    mainClass = vim.fn.input("Main class (com.example.Main): ", ""),
    projectName = vim.fn.fnamemodify(vim.fn.getcwd(), ":t"),
    stopOnEntry = false,
    args = vm.fn.input("Program arguments: ", ""),
  },
}
