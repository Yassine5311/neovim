...
-- Better delete (don't pollute register) — ThePrimeagen
map({ "n", "x" }, "x", '"_x', { desc = "Delete char (no register)" })
map({ "n", "x" }, "X", '"_X', { desc = "Delete char backwards (no register)" })

-- Select all
map("n", "<leader>sa", "gg<S-v>G", { desc = "Select all" })

-- Duplicate line/selection (AstroNvim)
map("n", "<leader>yd", '"ayy"ap', { desc = "Duplicate line" })
map("x", "<leader>yd", '"ay\'>\'ap', { desc = "Duplicate selection", silent = true })

-- Join lines without moving cursor (LazyVim-adjacent)
map("n", "J", "mzJ`z", { desc = "Join lines" })

-- Keep cursor centered during search & half-page scroll — ThePrimeagen
...
-- ── Quickfix / Location list (with Trouble integration) ──
...
-- Navigate quickfix/trouble items (LazyVim pattern)
map("n", "[q", function()
...
end, { desc = "Prev quickfix/trouble" })
map("n", "]q", function()
  if require("trouble").is_open() then
...
end, { desc = "Next quickfix/trouble" })
